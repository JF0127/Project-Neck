    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_SEND_AUDIO) {
            while (auto packet = audio_service_.PopPacketFromSendQueue()) {
                if (user_audio_callback_) {
                    user_audio_callback_(*packet);
                }
        ...
        if (strcmp(type->valuestring, "notify") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                if (cJSON_IsString(text)) {
                    ...
                    Schedule([display, message = std::move(message), glyphs = std::move(glyphs),
                              bpp]() {
            ...
            if (cJSON_IsString(text)) {
                std::string message(text->valuestring);
                if (user_text_callback_) {
                    user_text_callback_(message);
                }
                std::vector<TextGlyph> glyphs;
                uint8_t bpp = 0;
                if (!TextGlyphPayload::Parse(root, glyphs, bpp)) {
...
void Application::RegisterUserTextCallback(std::function<void(const std::string&)> callback) {
...
void Application::RegisterUserAudioCallback(
    std::function<void(const AudioStreamPacket&)> callback) {
    user_audio_callback_ = std::move(callback);
}

void Application::RegisterRobotAudioCallback(
    std::function<void(const AudioStreamPacket&)> callback) {
